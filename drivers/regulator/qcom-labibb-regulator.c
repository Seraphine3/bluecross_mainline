...
static irqreturn_t labibb_sc_err_handler(int irq, void *_reg)
{
	...
	if (!in_sc_err) {
		...
		do {
			...
			ret = regmap_bulk_read(labibb_reg->regmap, reg,
						&val, 1);
			if (ret < 0) {
				dev_err(labibb_reg->dev,
					"Read failed, ret=%d\n", ret);
				return IRQ_HANDLED;
			}
			reg_en = !!(val & LABIBB_CONTROL_ENABLE);
		...
		if (labibb_reg->vreg_enabled && !reg_en) {
			dev_dbg(labibb_reg->dev, "%s has been disabled by SCP\n",
