...
#define GPI_LOG(gpi_dev, fmt, ...) do { \
		dev_info(gpi_dev->dev, "%s: " fmt, __func__, ##__VA_ARGS__); \
	} while (0)
...
#define GPII_INFO(gpii, ch, fmt, ...) do { \
		dev_info(gpii->gpi_dev->dev, "%s:%u:%s: " fmt, gpii->label, ch, \
			__func__, ##__VA_ARGS__); \
	} while (0)
#define GPII_ERR(gpii, ch, fmt, ...) do { \
		dev_err(gpii->gpi_dev->dev, "%s:%u:%s: " fmt, gpii->label, ch, \
		       __func__, ##__VA_ARGS__); \
	} while (0)
...
#define GPII_REG(gpii, ch, fmt, ...) do { \
		dev_info(gpii->gpi_dev->dev, "%s:%u:%s: " fmt, gpii->label, \
